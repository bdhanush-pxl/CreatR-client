...
function Dashboard() {
  ...
  const [isRecentPostsLoading, setIsRecentPostsLoading] = useState(false);
  const [isRecentActivityLoading, setIsRecentActivityLoading] = useState(false);
  const [isDailyViewsLoading, setIsDailyViewsLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchRecentPosts = async () => {
      ...
      try {
        const res = await fetch(`${API_URL}/api/dashboard/posts-with-analytics?limit=5`, {
          method: "GET",
          credentials: "include",
        });
        const data = await res.json();
    ...
    const fetchRecentActivity = async () => {
      ...
      try {
        const res = await fetch(`${API_URL}/api/dashboard/recent-activity?limit=8`, {
          method: "GET",
          credentials: "include",
        });
        const data = await res.json();
    ...
    const fetchDailyViews = async () => {
      setIsDailyViewsLoading(true);
      try {
        const res = await fetch(`${API_URL}/api/dashboard/daily-views`, {
          method: "GET",
          credentials: "include",
        });
        const data = await res.json();
        setDailyViewsData(data.data);
        console.log("Daily views data:", data.data);
      } catch (error) {
        console.error("Error fetching daily views:", error);
      } finally {
        setIsDailyViewsLoading(false);
      }
    };
  ...
  // Loading states
  if (isAnalyticsLoading || isRecentPostsLoading || isRecentActivityLoading || isDailyViewsLoading) {
    return (
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="card-glass">
            ...
            <CardContent>
              <DailyViewsChart data={dailyViewsData} isLoading={isDailyViewsLoading} />
            </CardContent>
          </Card>
        </div>
