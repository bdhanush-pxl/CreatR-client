...
import { uploadToImageKit, buildTransformationUrl } from "../lib/imagekit.js";
...
const ImageUploadModal = ({ isOpen, onClose, onImageSelect, title = "Upload & Transform Image", }) => {
    ...
    const onDrop = async (acceptedFiles) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                console.log("Uploaded image:" + result.data.url + " width:" + result.data.width + " height:" + result.data.height);
    ...
    // // Probes the transformed URL by trying to load it as an image.
    // // ImageKit processes AI operations (like bgremove) asynchronously —
    // // the URL exists immediately but the transformation isn't applied
    // // until the job finishes. We retry with increasing delays until the
    // // image actually loads successfully.
    const waitForImageReady = (url, maxAttempts = 12, baseDelay = 2000) => {
        return new Promise((resolve, reject) => {
            ...
            const tryLoad = () => {
                ...
                img.onerror = () => {
                    ...
                    // Exponential backoff: 2s, 3s, 4s ... capped at 6s
                    const delay = Math.min(baseDelay + (attempt - 1) * 1000, 6000);
                    setTimeout(tryLoad, delay);
                };

                // Cache-bust so the browser doesn't serve a stale failed response
                img.src = `${url}?retry=${attempt}&t=${Date.now()}`;
            };

            // Give ImageKit a 2s head-start before the first probe
            setTimeout(tryLoad, baseDelay);
        });
    };
    ...
    const applyTransformations = async () => {
        ...
        try {
            ...
            if (watchedValues.backgroundRemoved) {
                transformationChain.push({ effect: "bgremove" });
            }
            ...
            if (watchedValues.textOverlay?.trim()) {
                transformationChain.push({
                    ...
                    overlayTextGravity: watchedValues.textPosition,
                });
            }
            ...
            if (watchedValues.backgroundRemoved) {
                // Wait the full 25s for ImageKit's bgremove AI to finish.
                await new Promise((r) => setTimeout(r, 25000));

                // Force a fresh download into the browser cache BEFORE setting
                // the img src. Without this, the browser serves a stale cached
                // copy of the original (from when it first encountered this URL),
                // causing the image to appear unchanged for a few seconds after
                // the toast fires.
                await fetch(transformedUrl, { cache: "reload" });
            }
    ...
    return (
        <Dialog open={isOpen} onOpenChange={handleClose}>
            <DialogContent className="!max-w-6xl !h-[90vh] overflow-y-auto bg-black border-slate-700 backdrop-blur-md">
                <DialogHeader>
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <TabsList className="grid w-full grid-cols-2 bg-[#2d2d2d] rounded-lg p-1">
                        <TabsTrigger value="upload" className="text-white data-[state=active]:bg-[#2f2f2f] data-[state=active]:text-white rounded">Upload</TabsTrigger>
                        <TabsTrigger value="transform" className="text-slate-400 disabled:opacity-50 data-[state=active]:bg-[#262626] data-[state=active]:text-white rounded" disabled={!uploadedImage}>
                            Transform
                        </TabsTrigger>
                    </TabsList>
                    ...
                    <TabsContent value="transform" className="space-y-6">
                        <div className="grid lg:grid-cols-2 gap-6 max-h-[70vh] overflow-y-auto">
                            <div className="space-y-6">
                                <div className="space-y-4">
                                    <h3 className="text-lg font-semibold text-white flex items-center sticky top-0 bg-black z-10">
